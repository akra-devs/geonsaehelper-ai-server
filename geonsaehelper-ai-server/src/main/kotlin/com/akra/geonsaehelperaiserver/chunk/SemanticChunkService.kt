...
class SemanticChunkService(
    ...
    private fun buildSystemPrompt(roleInstructions: String?): String {
        val basePrompt = """
            ...
            4. 블록의 끝에서도 overlap으로 이어질 미완성 문장이나 dangling 구절이 있으면 제거합니다. 다음 블록이 이어받을 수 있도록 완전한 문장에서 청크를 마무리하고, 필요하면 마지막 문장을 통째로 제외하세요.
            5. 완성된 청크들을 순서대로 하나의 JSON 배열에 담아 출력합니다. 배열 외의 텍스트, 설명, 주석은 절대 추가하지 마세요.

            추가 지침:
            - 입력에 존재하지 않는 문장이나 표현을 절대 새로 만들지 마세요.
            - 목록과 표를 다룰 때는 헤더와 항목이 분리되지 않도록 묶어 두되, 필요하면 항목 그룹별로 나누세요.
